...
def pre_process_fypp(args):
    ...
    # Check destination folder for preprocessing. if not 'fftpack-fpm', it is assumed to be the root folder.
...
def deploy_fftpack_fpm(with_ilp64):
    """create the fftpack-fpm folder for backwards compatibility (to be deprecated)
    """
    import shutil
    prune = (
    ...
    if with_ilp64:
        base_folder = 'fftpack-fpm-ilp64'
    else:
        base_folder = 'fftpack-fpm'
                ...
                if file not in prune:
                    if file.endswith((".f90", ".F90", ".dat", ".npy", ".c", ".h")):
                        shutil.copy2(os.path.join(root, file),
                                     base_folder+os.sep+folder+os.sep+file)
    recursive_copy('src')
    recursive_copy('test')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Preprocess fftpack source files.')
    # fypp arguments
    parser.add_argument("--vmajor", type=int, default=0,
                        help="Project Version Major")
    parser.add_argument("--vminor", type=int, default=5,
                        help="Project Version Minor")
    parser.add_argument("--vpatch", type=int, default=0,
    ...
    parser.add_argument("--maxrank", type=int, default=4,
                        help="Set the maximum allowed rank for arrays")
    parser.add_argument("--with_qp", action='store_true',
                        help="Include WITH_QP in the command")
    parser.add_argument("--with_xdp", action='store_true',
                        help="Include WITH_XDP in the command")
    parser.add_argument("--with_ilp64", action='store_true',
                        help="Include WITH_ILP64 to build 64-bit integer BLAS/LAPACK")
    parser.add_argument("--lnumbering", action='store_true',
                        help="Add line numbering in preprocessed files")
    parser.add_argument("--deploy_fftpack_fpm", action='store_true',
                        help="create the fftpack-fpm folder")
    ...
    if args.deploy_fftpack_fpm:
        deploy_fftpack_fpm(args.with_ilp64)
    # ==========================================
    # build using fpm
    if args.build:
