module test_fftpack_utils
   ...
   use fftpack, only: dp, fft, ifft, fftshift, ifftshift, fftfreq, rfftfreq
...
contains
   ...
   subroutine test_fftshift_complex(error)
      ...
      call check(error, all(fftshift(xeven) == [complex(kind=dp) :: 3, 4, 1, 2]), &
                 "all(fftshift(xeven) == [complex(kind=dp) :: 3, 4, 1, 2]) failed.")
      if (allocated(error)) return
      call check(error, all(fftshift(xodd) == [complex(kind=dp) :: 4, 5, 1, 2, 3]), &
                 "all(fftshift(xodd) == [complex(kind=dp) :: 4, 5, 1, 2, 3]) failed.")
   ...
   subroutine test_fftshift_real(error)
      ...
      call check(error, all(fftshift(xeven) == [real(kind=dp) :: 3, 4, 1, 2]), &
                 "all(fftshift(xeven) == [real(kind=dp) :: 3, 4, 1, 2]) failed.")
      if (allocated(error)) return
      call check(error, all(fftshift(xodd) == [real(kind=dp) :: 4, 5, 1, 2, 3]), &
                 "all(fftshift(xodd) == [real(kind=dp) :: 4, 5, 1, 2, 3]) failed.")
   ...
   subroutine test_ifftshift_complex(error)
      ...
      call check(error, all(ifftshift(xeven) == [complex(kind=dp) ::(i, i=1, 4)]), &
                 "all(ifftshift(xeven) == [complex(kind=dp) ::(i, i=1, 4)]) failed.")
      if (allocated(error)) return
      call check(error, all(ifftshift(xodd) == [complex(kind=dp) ::(i, i=1, 5)]), &
                 "all(ifftshift(xodd) == [complex(kind=dp) ::(i, i=1, 5)]) failed.")
   ...
   subroutine test_ifftshift_real(error)
      ...
      call check(error, all(ifftshift(xeven) == [real(kind=dp) ::(i, i=1, 4)]), &
                 "all(ifftshift(xeven) == [real(kind=dp) ::(i, i=1, 4)]) failed.")
      if (allocated(error)) return
      call check(error, all(ifftshift(xodd) == [real(kind=dp) ::(i, i=1, 5)]), &
                 "all(ifftshift(xodd) == [real(kind=dp) ::(i, i=1, 5)]) failed.")
   ...
   subroutine test_fftfreq_2(error)
      ...
      real(dp), parameter :: twopi = 8*atan(1.0_dp)  ! 2*pi
      complex(dp), parameter :: imu = (0, 1)  ! imaginary unit

      integer, parameter :: n = 128
      integer :: i
      complex(dp), dimension(n) :: xvec, xfou
      real(dp), dimension(n) :: xtrue
