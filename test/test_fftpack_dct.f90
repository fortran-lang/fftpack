...
contains
   ...
   subroutine test_classic_dct(error)
      ...
      real(kind=dp) :: w(3*4 + 15), w2(3*4 + 15)
      ...
      call dcosti(4, w)
      call dcost(4, x, w)
      call check(error, sum(abs(x - [real(kind=dp) :: 15, -4, 0, -1.0000000000000009_dp])) < eps, &
                 "`dcost` failed.")
      ...
      call dcost(4, x, w)
      call check(error, sum(abs(x/(2*3) - [real(kind=dp) :: 1, 2, 3, 4])) < eps, &
                 "2nd `dcost` failed.")
      if (allocated(error)) return
      ...
      call dcosqi(4, w)
      call dcosqf(4, x, w)
      call check(error, sum(abs(x - [11.999626276085150_dp, -9.1029432177492193_dp, &
                                     2.6176618435106480_dp, -1.5143449018465791_dp])) < eps, &
                 "`dcosqf` failed.")
      if (allocated(error)) return
      ...
      call dcosqb(4, x, w)
      call check(error, sum(abs(x/(4*4) - [real(kind=dp) :: 1, 2, 3, 4])) < eps, &
                 "`dcosqb` failed.")
      if (allocated(error)) return
   ...
   subroutine test_modernized_dct(error)
      ...
      ! DCT-1
      call check(error, sum(abs(dct(x, 2, 1) - [0.0_dp, 18.0_dp])) < eps, "`dct(x,2,1)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(dct(x, 3, 1) - dct(x, type=1))) < eps, "`dct(x,3,1)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(dct(x, 4, 1) - [real(kind=dp) :: -3, -3.0000000000000036_dp, 15, 33])) < eps, &
                 "`dct(x,4,1)` failed.")
      !DCT-2
      x = [9, -9, 3]
      call check(error, sum(abs(dct(x, 3, 2) - [12.0_dp, 20.784609690826525_dp, 60.0_dp])) < eps, &
                 "`dct(x,3,2)` failed.")
      call check(error, sum(abs(dct(x, 3) - [12.0_dp, 20.784609690826525_dp, 60.0_dp])) < eps, &
                 "`dct(x,3)` failed.")
      call check(error, sum(abs(dct(x, 4, 2) - [12.0_dp, 14.890858416882008_dp, 42.426406871192853_dp, &
                                                58.122821125684993_dp])) < eps, "`dct(x,4,2)` failed.")

      ! DCT-3
      x = [9, -9, 3]
      call check(error, sum(abs(dct(x, 2, 3) - [-3.7279220613578570_dp, 21.727922061357859_dp])) < eps, &
                 "`dct(x,2,3)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(dct(x, 3, 3) - dct(x, type=3))) < eps, &
                 "`dct(x,3,3)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(dct(x, 4, 3) - [-3.3871908980838743_dp, -2.1309424696909023_dp, &
   ...
   subroutine test_modernized_idct(error)
      ...
      if (allocated(error)) return
      call check(error, sum(abs(idct(dct(x, type=1), 2, 1)/(2*1) - [5.5_dp, 9.5_dp])) < eps, &
                 "`idct(dct(x,type=1), 2, 1)/(2*1)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(idct(dct(x, 2, 1), 4, 1)/(2*3) - [0.16666666666666666_dp, 0.33333333333333331_dp, &
                                                                  0.66666666666666663_dp, 0.83333333333333315_dp])) < eps, &
      ...
      if (allocated(error)) return
      call check(error, sum(abs(idct(dct(x, type=2), n=2) - [22.156460020898692_dp, 57.843539979101308_dp])) < eps, &
                 "`idct(dct(x, type=2),n=2)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(idct(dct(x, n=2, type=2), n=4) - [6.7737481404944937_dp, 9.8352155994152106_dp, &
                                                                  14.164784400584789_dp, 17.226251859505506_dp])) < eps, &
      ...
      call check(error, sum(abs(idct(dct(x, type=3), n=2, type=3)/(4*2) - &
                                [1.4483415291679655_dp, 7.4608849947753271_dp])) < eps, &
                 "`idct(dct(x, type=3), n=2, type=3)/(4*2)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(idct(dct(x, n=2, type=3), n=4, type=3)/(4*4) - &
                                [0.5_dp, 0.70932417358418376_dp, 1.0_dp, 0.78858050747473762_dp])) < eps, &
